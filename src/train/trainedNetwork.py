...
device = torch.device('cuda:2') #currently using GPU-1
torch.cuda.set_device(2) #currently using GPU-1
numTargets = 6
numInstances = 500
...
#%% 
total_iter = 75
for numLayers in np.arange(16,51):
    ...
    loss_1_store=np.zeros((20,10,total_iter))
    loss_2_store=np.zeros((20,10,total_iter))
    loss_3_store=np.zeros((20,10,total_iter))
    ...
    for idxsnr in np.arange(7,20):
        for idxRealization in np.arange(10):
            ...
            #                                   y,nlp_configObj)
                ...
                #%% IFISTA 
            model1=LISTA_Ifista( numLayers=numLayers, scale_mag=scale_mag,step_size=1e-4,gpu_dev = device, 
            ...
            for iter in range(total_iter):
                ...
                loss.backward()
                loss_1_store[idxsnr,idxRealization,iter]=loss.detach().cpu().numpy()
                optimizer_1.step()
            end_time = timer()
            ...
            MSE_1[idxsnr,idxRealization] = np.sqrt(np.sum(np.abs(y_ret_1[idxsnr,idxRealization,:,:,:]
            ...
            for iter in range(total_iter):
                ...
                loss.backward()
                loss_2_store[idxsnr,idxRealization,iter]=loss.detach().cpu().numpy()
                optimizer_2.step()
            end_time = timer()
            ...
            MSE_2[idxsnr,idxRealization] = np.sqrt(np.sum(np.abs(y_ret_2[idxsnr,idxRealization,:,:,:]
                                                -np.squeeze(y.detach().cpu().numpy()))**2.0))
            ...
            for iter in range(total_iter):
                ...
                loss.backward()
                loss_3_store[idxsnr,idxRealization,iter]=loss.detach().cpu().numpy()
                optimizer_3.step()
            end_time = timer()
            ...
            MSE_3[idxsnr,idxRealization] = np.sqrt(np.sum(np.abs(y_ret_3[idxsnr,idxRealization,:,:,:]
                                                -np.squeeze(y.detach().cpu().numpy()))**2.0))
    ...
    hdf5storage.savemat('/research/nfs_ertin_1/nithin_data/mod/blip/python/src/unrolled/simulaed_data/matFinal_trainedNet_{}.mat'.format(numLayers), vals_to_save , format='7.3')
#%% 
# Z=Z+1e-5
