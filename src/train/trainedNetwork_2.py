    ...
    for idxsnr in np.arange(12,13):
        for idxRealization in np.arange(numInstances):
            ...
            #                                   y,nlp_configObj)
                ...
                #%% IFISTA 
            # model1=LISTA_Ifista( numLayers=numLayers, scale_mag=scale_mag,step_size=1e-6,gpu_dev = device, 
            #              actfunc="shrink",  angleDopplerOp=op1,tiedRegularization=True,fixRegularizer = False)
            # model1.to(device)
            #%% FISTA 
            ...
            model2.to(device)
            #%% ISTA 
            model3=LISTA( numLayers=numLayers, scale_mag=scale_mag,step_size=1e-2,gpu_dev = device, 
                        actfunc="shrink",  angleDopplerOp=op1,tiedRegularization=True,fixRegularizer = False)
            model3.to(device)

            # optimizer_1 = torch.optim.Adam(model1.parameters(), lr=1e-5)
            # scheduler_1 = torch.optim.lr_scheduler.MultiStepLR(optimizer_1, [total_iter//3, total_iter//2], gamma=0.5)
            optimizer_2 = torch.optim.Adam(model2.parameters(), lr=1e-3)
            scheduler_2 = torch.optim.lr_scheduler.MultiStepLR(optimizer_2, [total_iter//3, total_iter//2], gamma=0.5)
            optimizer_3 = torch.optim.Adam(model3.parameters(), lr=1e-3)
            ...
            # MSE_1[0,idxRealization] = np.sqrt(np.sum(np.abs(y_ret_1[0,idxRealization,:,:,:]
            ...
            MSE_2[0,idxRealization] = np.sqrt(np.sum(np.abs(y_ret_2[0,idxRealization,:,:,:]
                                                -np.squeeze(y.detach().cpu().numpy()))**2.0))
            ...
            MSE_3[0,idxRealization] = np.sqrt(np.sum(np.abs(y_ret_3[0,idxRealization,:,:,:]
                                                -np.squeeze(y.detach().cpu().numpy()))**2.0))

    f1=  '/research/nfs_ertin_1/nithin_data/mod/blip/python/src/unrolled/simulaed_data/'
    vals_to_save = {'y_ret_2':y_ret_2,'Z_2':Z_2,'times_2':times_2,'MSE_2':MSE_2,
        'y_ret_3':y_ret_3,'Z_3':Z_3,'times_3':times_3,'MSE_3':MSE_3}

    hdf5storage.savemat('/research/nfs_ertin_1/nithin_data/mod/blip/python/src/unrolled/simulaed_data/matFinal_trainedNet_trainLambda_reg{}.mat'.format(numLayers), vals_to_save , format='7.3')
#%% 
# Z=Z+1e-5
