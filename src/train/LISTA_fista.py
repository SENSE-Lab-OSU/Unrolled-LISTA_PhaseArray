...
class LISTA_fista(nn.Module):
    ...
    def forward(self, Y):
        ...
        Z0 = torch.zeros_like(self.angleDopplerOp.adjoint_mat(y))
        Z1 = self.applyActFunc(Y1-torch.pow(self.stepW[0],2)*self.angleDopplerOp.least_squares_gram(
                                    Y1,y.detach()),self.lam[0]*self.stepW[0])
        ...
        for L in range(self.NL-1):
            if self.tiedRegularization == False:
                ...
                Z1 = self.applyActFunc(Y1-torch.pow(self.stepW[L+1],2)*self.angleDopplerOp.least_squares_gram(cpi_header,
                    sensor,Y1,y.detach(),nlp_opts),torch.pow(self.lam[L+1]*self.stepW[L+1],2))
                alphaStep1=(1+torch.sqrt(1+4*torch.pow(alphaStep0,2)))/2.0
                Y1 = Z1+ (alphaStep0-1)/(alphaStep1) * (Z1-Z0)
            else:
                alphaStep0=torch.empty_like(alphaStep1).copy_(alphaStep1)
                Z0 = torch.empty_like(Z1).copy_(Z1)
                Z1 = self.applyActFunc(Y1-torch.pow(self.stepW[L+1],2)*self.angleDopplerOp.least_squares_gram(
                    Y1,y.detach()),torch.pow(self.lam[0]*self.stepW[L+1],2))
                alphaStep1=(1+torch.sqrt(1+4*torch.pow(alphaStep0,2)))/2.0
                Y1 = Z1+ (alphaStep0-1)/(alphaStep1) * (Z1-Z0)             
        ...
        sol = self.angleDopplerOp.forward_mat(Z1)
        sol_debias = Z1 + 1/(self.angleDopplerOp.A_angle.shape[0]*
        ...
        reg = torch.pow(self.lam[0],2)*self.proxOp.group_reg(Z1)
        return Z1,sol,reg,sol_debias,y_sol_debias
