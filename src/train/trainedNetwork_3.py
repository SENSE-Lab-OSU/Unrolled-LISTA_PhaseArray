...
#%% 
total_iter = 100
for numLayers in np.arange(9,14):
    Z_1 = np.zeros((1,numInstances,2,nlp_configObj.num_vel,
                    nlp_configObj.num_az*nlp_configObj.num_el),dtype=np.complex64)
    Z_2 = np.zeros((1,numInstances,2,nlp_configObj.num_vel,
                     nlp_configObj.num_az*nlp_configObj.num_el),dtype=np.complex64)
    Z_3 = np.zeros((1,numInstances,2,nlp_configObj.num_vel,
                     nlp_configObj.num_az*nlp_configObj.num_el),dtype=np.complex64)
    y_ret_1 = np.zeros((1,numInstances,2,32,24),dtype=np.complex64)
    y_ret_2 = np.zeros((1,numInstances,2,32,24),dtype=np.complex64)
    y_ret_3 = np.zeros((1,numInstances,2,32,24),dtype=np.complex64)
    times_1 = np.zeros((1,numInstances))
    times_2 = np.zeros((1,numInstances))
    times_3 = np.zeros((1,numInstances))
    MSE_1 = np.zeros((1,numInstances))
    MSE_2 = np.zeros((1,numInstances))
    MSE_3 = np.zeros((1,numInstances))
    G_1 = np.zeros((1,numInstances))
    G_2 = np.zeros((1,numInstances))
    G_3 = np.zeros((1,numInstances))
    ...
    for idxsnr in np.arange(12,13):
        for idxRealization in np.arange(numInstances):
            y = torch.unsqueeze(torch.tensor(X['YSave'][idxRealization,:,:,:] 
                        + X['noiseaddSave'][idxsnr,idxRealization,:,:,:],dtype=torch.complex64),0)
            y=y.to(device)
            print('{} snr index,{} realization,{} num layers'.format(idxsnr,idxRealization,numLayers))
            #%%
            ...
            model1.to(device)
            #%% FISTA 
            model2=LISTA_fista( numLayers=numLayers, scale_mag=scale_mag,step_size=1e-2,gpu_dev = device, 
                        actfunc="shrink",  angleDopplerOp=op1,tiedRegularization=True,fixRegularizer = True)
            model2.to(device)
            #%% ISTA 
            model3=LISTA( numLayers=numLayers, scale_mag=scale_mag,step_size=5e-2,gpu_dev = device, 
                        actfunc="shrink",  angleDopplerOp=op1,tiedRegularization=True,fixRegularizer=True)
            model3.to(device)
            ...
            optimizer_2 = torch.optim.Adam(model2.parameters(), lr=1e-4)
            scheduler_2 = torch.optim.lr_scheduler.MultiStepLR(optimizer_2, [total_iter//3, total_iter//2], gamma=0.5)
            optimizer_3 = torch.optim.Adam(model3.parameters(), lr=1e-4)
            ...
            #     loss=torch.sum(torch.pow(torch.abs(y_ret_1_temp-y),2))
            #     loss.backward()
            #     loss_1_store[0,idxRealization,iter]=loss.detach().cpu().numpy()
            #     optimizer_1.step()
            # end_time = timer()
            
            # Z_1[0,idxRealization,:,:,:]=np.squeeze(Z_1_temp.detach().cpu().numpy())
            # y_ret_1[0,idxRealization,:,:,:] = np.squeeze(y_ret_1_temp.detach().cpu().numpy())
            # times_1[0,idxRealization] = end_time-start_time
            # MSE_1[0,idxRealization] = 0.5*(np.sum(np.abs(y_ret_1[0,idxRealization,:,:,:]
            #                                     -np.squeeze(y.detach().cpu().numpy()))**2.0))
            
            
            start_time=timer()
            for iter in range(total_iter):
                optimizer_2.zero_grad()
                Z_2_temp,y_ret_2_temp,reg_2_temp = model2.forward(Y)
                loss=torch.sum(torch.pow(torch.abs(y_ret_2_temp-y),2)) 
                loss.backward()
                loss_2_store[0,idxRealization,iter]=loss.detach().cpu().numpy()
                optimizer_2.step()
                # print('Fista {} iter,loss={}, reg={}, total={}'.format(iter,loss.detach().cpu().numpy(),reg_2_temp.detach().cpu().numpy(), 
                #                                                        0.5*(loss.detach().cpu().numpy())+reg_2_temp.detach().cpu().numpy()))
            end_time = timer()
            
            Z_2[0,idxRealization,:,:,:]=np.squeeze(Z_2_temp.detach().cpu().numpy())
            y_ret_2[0,idxRealization,:,:,:] =np.squeeze(y_ret_2_temp.detach().cpu().numpy())
            times_2[0,idxRealization] = end_time-start_time
            MSE_2[0,idxRealization] = 0.5*(np.sum(np.abs(y_ret_2[0,idxRealization,:,:,:]
                                                -np.squeeze(y.detach().cpu().numpy()))**2.0)) 
            G_2[0,idxRealization] = reg_2_temp.detach().cpu().numpy()
            
            start_time=timer()
            for iter in range(total_iter):
                optimizer_3.zero_grad()
                Z_3_temp,y_ret_3_temp,reg_3_temp = model3.forward(Y)
                loss=torch.sum(torch.pow(torch.abs(y_ret_3_temp-y),2))
                loss.backward()
                loss_3_store[0,idxRealization,iter]=loss.detach().cpu().numpy()
                optimizer_3.step()
                print('Ista {} iter,loss={}, reg={},total={},stepW={}'.format(iter,loss.detach().cpu().numpy(),reg_3_temp.detach().cpu().numpy(),
                                                                     0.5*(loss.detach().cpu().numpy())+reg_3_temp.detach().cpu().numpy(),
                                                                     torch.tensor(model3.stepW).detach().cpu().numpy()))
            end_time = timer()
            
            Z_3[0,idxRealization,:,:,:]=np.squeeze(Z_3_temp.detach().cpu().numpy())
            y_ret_3[0,idxRealization,:,:,:] =np.squeeze(y_ret_3_temp.detach().cpu().numpy())
            times_3[0,idxRealization] = end_time-start_time
            MSE_3[0,idxRealization] = 0.5*(np.sum(np.abs(y_ret_3[0,idxRealization,:,:,:]
                                                -np.squeeze(y.detach().cpu().numpy()))**2.0))
            G_3[0,idxRealization] = reg_3_temp.detach().cpu().numpy()
    ...
    vals_to_save = {'y_ret_2':y_ret_2,'Z_2':Z_2,'times_2':times_2,'MSE_2':MSE_2,'G_2':G_2,'stepW_2':stepW_2,
        'y_ret_3':y_ret_3,'Z_3':Z_3,'times_3':times_3,'MSE_3':MSE_3,'G_3':G_3,'stepW_3':stepW_3}

    hdf5storage.savemat('/research/nfs_ertin_1/nithin_data/mod/blip/python/src/unrolled/simulaed_data/matFinal_trainedNet_fixLambda_converge{}.mat'.format(numLayers)
                        , vals_to_save , format='7.3')
#%% 
# Z=Z+1e-5
